...
const AuthProvider = ({ children }) => {
  ...
  const loginAction = ({ usernameLogin, playerID }) => {
    axios.post('http://mpsplayerportal-server.vercel.app/user/logsession', {
      user: usernameLogin,
      userID: playerID,
    })
      .then(function (response) {
        ...
        window.location.replace(`http://mpsplayerportal-client.vercel.app/CharacterSelection/:${playerID}`);
        return;
      })
  ...
  const logOut = (event) => {
    axios.delete('http://mpsplayerportal-server.vercel.app/user/logout', {
